...
public interface LedgerTransactionRepository extends JpaRepository<LedgerTransactionEntity, String> {
    ...
    @Query("SELECT MAX(e.updatedAt) FROM LedgerTransactionEntity e WHERE e.libraryId = :libraryId")
    ...
    @Modifying
    @Query("UPDATE LedgerTransactionEntity e SET e.deletedAtMillis = :deletedAtMillis, e.updatedAt = :updatedAt " +
