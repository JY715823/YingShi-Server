...
public interface LedgerRecurringRuleRepository extends JpaRepository<LedgerRecurringRuleEntity, String> {
    ...
    @Query("SELECT MAX(e.updatedAt) FROM LedgerRecurringRuleEntity e WHERE e.libraryId = :libraryId")
    ...
    @Modifying
    @Query("UPDATE LedgerRecurringRuleEntity e SET e.deletedAtMillis = :deletedAtMillis, e.updatedAt = :updatedAt " +
