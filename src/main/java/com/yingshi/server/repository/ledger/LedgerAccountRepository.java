...
public interface LedgerAccountRepository extends JpaRepository<LedgerAccountEntity, String> {
    ...
    @Query("SELECT MAX(e.updatedAt) FROM LedgerAccountEntity e WHERE e.libraryId = :libraryId")
    ...
    @Modifying
    @Query("UPDATE LedgerAccountEntity e SET e.deletedAtMillis = :deletedAtMillis, e.updatedAt = :updatedAt " +
