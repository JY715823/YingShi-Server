package com.yingshi.server.common.exception;

public enum ErrorCode {
    ALBUM_ASSIGNMENT_INVALID,
    ALBUM_NOT_FOUND,
    AUTH_INVALID_CREDENTIALS,
    AUTH_ACCOUNT_LOCKED,
    AUTH_LOGIN_CHALLENGE_INVALID,
    AUTH_LOGIN_CODE_EXPIRED,
    AUTH_LOGIN_CODE_INVALID,
    AUTH_LOGIN_CODE_RATE_LIMITED,
    AUTH_LOGIN_CODE_RESEND_TOO_FAST,
    AUTH_LOGIN_CODE_SEND_FAILED,
    AUTH_REMEMBERED_LOGIN_EXPIRED,
    AUTH_REMEMBERED_LOGIN_INVALID,
    AUTH_SESSION_INVALID,
    AUTH_TOKEN_EXPIRED,
    AUTH_UNAUTHORIZED,
    COMMENT_NOT_FOUND,
    COMMENT_SCOPE_MISMATCH,
    COMMENT_TARGET_NOT_FOUND,
    DELETE_CONFLICT,
    FORBIDDEN,
    MEDIA_NOT_FOUND,
    MEDIA_ALREADY_DELETED,
    NOT_FOUND,
    SMALL_ALBUM_COVER_INVALID,
    SMALL_ALBUM_ALREADY_DELETED,
    SMALL_ALBUM_MEDIA_ORDER_INVALID,
    SMALL_ALBUM_NOT_FOUND,
    REMOVE_FROM_TRASH_CONFLICT,
    RESTORE_CONFLICT,
    TRASH_ITEM_NOT_FOUND,
    UNDO_REMOVE_EXPIRED,
    UPLOAD_ALREADY_COMPLETED,
    UPLOAD_CONTENT_TYPE_MISMATCH,
    UPLOAD_EXPIRED,
    UPLOAD_FILE_MISMATCH,
    UPLOAD_NOT_FOUND,
    UPLOAD_OBJECT_INVALID,
    UPLOAD_OBJECT_MISMATCH,
    UPLOAD_SIZE_MISMATCH,
    UPLOAD_STORAGE_ERROR,
    CHAT_IMPORT_EMPTY_FILE,
    CHAT_IMPORT_INVALID_FORMAT,
    CHAT_IMPORT_READ_ERROR,
    CHAT_IMPORT_INVALID_ARCHIVE,
    CHAT_IMPORT_FAILED,
    CHAT_IMPORT_SIZE_EXCEEDED,
    CHAT_IMPORT_ZIP_BOMB_DETECTED,
    CHAT_IMPORT_PATH_TRAVERSAL,
    VALIDATION_ERROR,
    METHOD_NOT_ALLOWED,
    SERVER_ERROR
}
